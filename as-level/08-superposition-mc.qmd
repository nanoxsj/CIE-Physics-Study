...
**Answer: C.** Using $d\sin\theta = n\lambda$, for $n=3$, $\theta = 45^\circ$, so $d\sin45^\circ = 3\lambda$, giving $d = 3\lambda/\sin45^\circ = 3\sqrt{2}\lambda$. The maximum order occurs when $\sin\theta = 1$, so $n_{\text{max}} = d/\lambda = 3\sqrt{2} \approx 4.24$. Therefore the highest integer order is 4.
</details>
:::
